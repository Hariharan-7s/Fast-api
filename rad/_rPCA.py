...
def Dsoft(M, penalty):
    ...
    Returns
    -------
    M matrix with elements penalized when they violate this condition
    """
...
def SVT(M, penalty):
    ...
    -------
    S: numpy array 
       The singular value thresholded matrix
    Ds: numpy array
...
def RPCA(X, Lpenalty=-1, Spenalty=-1, verbose=True):
    ...
    while (itere < maxIter and diff > tol):
        ...
        if (verbose):
            print("Objective function: %4.8f  on previous iteration %d " %
                  (obj_prev, itere-1))
    ...
    if (converged):
        ...
        raise ValueError('Matrix values did not converge, failed to converge within '
